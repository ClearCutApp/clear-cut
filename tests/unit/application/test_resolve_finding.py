...
def test_a_transition_writes_a_new_row_and_preserves_the_old_one() -> None:
    ...
    use_case.execute("proj-1", "itm-1", Transition(TrackerState.IN_PROGRESS), _AT)
...
def test_execute_returns_the_new_version() -> None:
    ...
    result = use_case.execute("proj-1", "itm-1", Transition(TrackerState.CLEARED), _AT)
...
def test_notify_action_calls_notifier_exactly_once() -> None:
    ...
    use_case.execute("proj-1", "itm-1", Notify("producer requested a status update"), _AT)
...
def test_draft_email_action_fills_the_template_from_the_items_own_data() -> None:
    ...
    result = use_case.execute("proj-1", "itm-1", DraftEmail(), _AT)
...
def test_draft_email_action_never_calls_the_notifier() -> None:
    ...
    use_case.execute("proj-1", "itm-1", DraftEmail(), _AT)
...
def test_an_unknown_item_id_propagates_the_stores_error_unchanged() -> None:
    ...
    with pytest.raises(KeyError):
        use_case.execute("proj-1", "no-such-item", Transition(TrackerState.IN_PROGRESS), _AT)
...
def test_a_notifier_failure_does_not_lose_an_already_saved_transition() -> None:
    ...
    with pytest.raises(RuntimeError):
        raising_notifier.execute("proj-1", "itm-1", Notify("status update"), _AT)
...
def test_port_methods_are_called_positionally() -> None:
    ...
    transitioned = use_case.execute("proj-1", "itm-1", Transition(TrackerState.IN_PROGRESS), _AT)
    use_case.execute("proj-1", "itm-1", Notify("status update"), _AT)
